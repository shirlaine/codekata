...
def gifts(number)
  ...
  while number>0 do
    leftover_gifts= GIFTS.select {|index,gift| index<=number}
    keys = leftover_gifts.keys
    gift_array << leftover_gifts[keys[keys.length-1]]
    number =number- keys[keys.length-1]
  end
